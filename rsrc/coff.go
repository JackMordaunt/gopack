...
type DataEntry struct { // struct IMAGE_RESOURCE_DATA_ENTRY
	OffsetToData uint32
	Size1        uint32
	CodePage     uint32 //Fix: what value here? for now just using 0
	Reserved     uint32
}
...
// Values reverse-engineered from windres output; names from the internet.
// Couldn't find much on AMD64 unfortunately, but it works.
const (
...
var (
	...
	LANG_ENTRY = DirEntry{NameOrId: 0x0409} //Fix: language; what value should be here?
)
...
// Arch is an identifier for CPU architecture.
type Arch uint16

// Supported architectures.
const (
	X86 Arch = pe.IMAGE_FILE_MACHINE_I386
	// Sources:
	...
	// Fix: currently experimental -- not sure if something more doesn't need to be changed
...
func NewRDATA(arch Arch) *Coff {
	return &Coff{
		...
		[]Symbol{{
			...
			SectionNumber:  1,
			Type:           0, // Fix: wtf?
			StorageClass:   3, // Fix: is it ok? and uint8? and what does the value mean?
			AuxiliaryCount: 1,
...
func (coff *Coff) AddData(symbol string, data Sizer) error {
	if len(symbol) <= 8 {
		return fmt.Errorf("symbol too short: len %d", len(symbol))
	}
	if strings.Contains(symbol, "0") {
		return fmt.Errorf("symbol contains embeded zeroes: %q", symbol)
	}
...
func NewRSRC(arch Arch) *Coff {
	return &Coff{
		pe.FileHeader{
			...
			NumberOfSymbols:      1,
			SizeOfOptionalHeader: 0,
			Characteristics:      0x0104, // Fix: copied from windres.exe output, find out what should be here and why
		},
		pe.SectionHeader32{
			Name:            STRING_RSRC,
			Characteristics: 0x40000040, // "INITIALIZED_DATA MEM_READ" ?
...
func (coff *Coff) freezeRDATA() {
	...
	if err := binutil.Walk(coff, func(v reflect.Value, path string) error {
		...
		switch {
		case matcher.Find(path, regexp.MustCompile(`^/Data\[(\d+)\]$`)):
			n := matcher[0]
			coff.Symbols[1+n].Value = offset - diroff // Fix: is it ok?
		...
		case path == "/StringsHeader":
			stringsoff = offset
		case matcher.Find(path, regexp.MustCompile(`^/Strings\[(\d+)\]$`)):
			binary.LittleEndian.PutUint32(coff.Symbols[matcher[0]+1].Name[4:8], offset-stringsoff)
		}
		return freezeCommon2(v, &offset)
	}); err != nil {
		panic(fmt.Errorf("walking coff: %w", err))
	}
...
func (coff *Coff) freezeRSRC() {
	...
	if err := binutil.Walk(coff, func(v reflect.Value, path string) error {
		...
		matcher := Matcher{}
		switch {
		case matcher.Find(path, regexp.MustCompile("^/Dir/Dirs"+`\[(\d+)\]`+"$")):
			coff.Dir.DirEntries[matcher[0]].OffsetToData = MASK_SUBDIRECTORY | (offset - diroff)
		case matcher.Find(path, regexp.MustCompile("^/Dir/Dirs"+`\[(\d+)\]`+"/Dirs"+`\[(\d+)\]`+"$")):
			coff.Dir.Dirs[matcher[0]].DirEntries[matcher[1]].OffsetToData = MASK_SUBDIRECTORY | (offset - diroff)
		case matcher.Find(path, regexp.MustCompile("^/DataEntries"+`\[(\d+)\]`+"$")):
			direntry := <-leafwalker
			direntry.OffsetToData = offset - diroff
		case matcher.Find(path, regexp.MustCompile("^/DataEntries"+`\[(\d+)\]`+"/OffsetToData$")):
			coff.Relocations[matcher[0]].RVA = offset - diroff
		case matcher.Find(path, regexp.MustCompile("^/Data"+`\[(\d+)\]`+"$")):
			coff.DataEntries[matcher[0]].OffsetToData = offset - diroff
		}
